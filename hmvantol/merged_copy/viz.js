$( document ).ready(function() {
  ...
  function show_information(current_location) {
    ...
    d3.csv("timeline/"+current_location+".csv", function(data) {

      xy_values = []
      var parseDate = d3.time.format("%Y-%m-%d").parse;


      //Need to convert all NA to null - still not working...
      data.forEach(function(d) {
        if (d.x == "NA") {
          xy_values.push({x: null, y: null});
          d.x = null;
          d.y = null;
        }
        if (d.data1 == "NA") {
      ...
      w1 = d3.min(xy_values.map(function(d) { return Math.abs(d.x) || Infinity; }));
      h1 = d3.min(xy_values.map(function(d) { return Math.abs(d.y) || Infinity; }));
      w2 = d3.max(xy_values.map(function(d) { return Math.abs(d.x); }));
      h2 = d3.max(xy_values.map(function(d) { return Math.abs(d.y); }));
      var margin = {top:40, right:20, bottom:10, left:50};
      var width = {image: w1, plot: w1*2-30, total: w1*2, comp_img: h2};
      var height = {image: h1, plot: h1, comp_img: h2};
    ...
    defs.append("image")
      .attr("id", "satellite")
      .attr("xlink:href", "img/"+current_location+".png")
      .attr("width", width.comp_img)
      .attr("height", height.comp_img);
      ...
      var linel = d3.svg.line()
        ...
        .y(function(d) { return yl(d.data1); });
              
      var lineg = d3.svg.line()
        .x(function(d) { return x(d.date); })
        .y(function(d) { return yg(d.data2); });
      ...
      yl.domain(d3.extent(data, function(d) { return d.data1; }));
      yg.domain(d3.extent(data, function(d) { return d.data2; }));
  ...
  // 			x.domain(d3.extent(data, function(d) { return d.date; }));
  // 			yl.domain(d3.extent(data, function(d) { return d.insitu; }));
  // 			yg.domain(d3.extent(data, function(d) { return d.mining; }));
  ...
  // 				.append("circle")
		...
		// 			.attr("cy", function(d) { return yl(d.insitu); })
		// 			.attr("transform", "translate("+margin.left+",0)")
		// 			.attr("r", "10px")
